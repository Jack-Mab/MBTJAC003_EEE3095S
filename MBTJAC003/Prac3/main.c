...
uint32_t ADCtoCCR(uint32_t adc_value);
/* USER CODE END PFP */
...
void EXTI0_1_IRQHandler(void)
{
	...
	// Toggle LED7 frequency between 1 Hz and 2 Hz

	if (HAL_GetTick() - lastStateChangeTime > DEBOUNCE_INTERVAL) {
		delay_t = (delay_t == 500) ? 1000 : 500; // Only change the frequency if the state change is stable
	}
	lastStateChangeTime = HAL_GetTick();
...
uint32_t ADCtoCCR(uint32_t adc_val){
  // TODO: Calculate CCR val using an appropriate equation
	float normalized_adc = (float)adc_val / 4095.0;  // Normalize ADC value to [0, 1]
	uint32_t val = (uint32_t)(normalized_adc * 47999.0);  // Calculate CCR
	return val;
}
