...
static uint8_t patterns[] = {0b10101010, 0b01010101, 0b11001100, 0b00110011, 0b11110000, 0b00001111}; // The bit patterns to display
static uint16_t currentAddress = 0; // The address of the next pattern in EEEPROM
static uint8_t errorPattern = 0b00000001;
static uint8_t period = 0; // Used to alternate between the 1s period and the 0.5s period
#define PA0_Pressed (( GPIOA -> IDR & GPIO_IDR_0) == 0) // Check if button 0 is pressed
...
int main(void)
{
  ...
  while (1)
  {
	...
	if(PA0_Pressed)
	{
		uint16_t arr_val = TIM16 -> ARR; // Get the current ARR value

		if (period)
		{
			TIM16 -> ARR = arr_val * 2; // Double it to change the period from 0.5s to 1s
			period = 0;
		} else {
			TIM16 -> ARR = arr_val / 2; // Halve it to change the period from 1s to 0.5s
			period = 1;
		}
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t pattern = read_from_address(currentAddress); // read next value from SPI

	if (pattern == patterns[currentAddress])
	{
		GPIOB -> ODR = pattern;    // Write pattern to LEDs if it's the right pattern
	} else{
		GPIOB -> ODR = errorPattern; // Else, write the error pattern
	}
